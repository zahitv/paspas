...
export const Experience = () => {
  ...
  const download = () => {
    ...
    return new Promise((resolve) => {
      try {
        ...
        link.setAttribute('download', 'paspas.png');
  ...
  const downloadOptions = useControls('İndirme ayarları',
    {
      Arkaplan: false,
      'Resmi İndir': button(async () => {
        try {
          await download();

        } catch (error) {
          alert("İndirme başarısız oldu:");
          console.error("İndirme başarısız oldu:", error);
        }
      }),
    },
    {
  ...
  const values = useControls('Arkaplan',
    {
      renk: '#2d4967',
      Parıltılar: true
    },
  ...
  // Memoize the renk value
  const _color = useMemo(() => {
    if (typeof values.renk !== 'string') {
      let c = values.renk;
  ...
  // Apply the background renk to the body
  useEffect(() => {
    document.body.style.backgroundColor = _color;

    // Cleanup function to reset the background renk when the component unmounts
    return () => {
      document.body.style.backgroundColor = '';
    };
  }, [_color, downloadOptions.Arkaplan]);
